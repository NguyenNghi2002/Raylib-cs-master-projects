...
namespace Engine
{
    public class Transform
	{
			...
			get
			{
				...
				if (_positionDirty)
				{
					...
					else
					{
						...
						_position = Vector2.Transform(_localPosition,Parent._worldTransform);
		...
		{
			get => _rotation * Raylib.RAD2DEG;//MathHelper.ToDegrees(_rotation);
			set => SetRotation(value * Raylib.DEG2RAD);
		}
		...
		{
			get => _localRotation * Raylib.RAD2DEG;//MathHelper.ToDegrees(_localRotation);
			set => LocalRotation = value * Raylib.DEG2RAD;//MathHelper.ToRadians(value);
		}
		...
		public Matrix2D WorldInverseTransform
		{
			get
			{
				...
				{
					Matrix2D.Invert(_worldTransform, out _worldInverseTransform);
		...
		public Matrix2D WorldToLocalTransform
		{
			get
			{
				if (_worldToLocalDirty)
				{
					...
					{
						UpdateTransform();
						Matrix2D.Invert(Parent._worldTransform, out _worldToLocalTransform);
		...
		public Transform SetParent(Transform parent)
		{
			...
			_parent = parent;
			parent?.SetDirty(DirtyType.PositionDirty);
		...
		{
			return SetRotation(degrees * Raylib.DEG2RAD);
		}
		...
		{
			return SetLocalRotation(degrees * Raylib.DEG2RAD);
		}
		...
		void UpdateTransform()
		{
			if (hierarchyDirty != DirtyType.Clean)
			{
				...
				if (_localDirty)
				{
					...
					{
						_translationMatrix =  Matrix2D.CreateTranslation(_localPosition);
					...
					{
						_rotationMatrix =  Matrix2D.CreateRotation(_localRotation);
					...
					if (_localScaleDirty)
					{
						_scaleMatrix = Matrix2D.CreateScale(_localScale);
					...
					_localTransform = _scaleMatrix * _rotationMatrix *_translationMatrix;
