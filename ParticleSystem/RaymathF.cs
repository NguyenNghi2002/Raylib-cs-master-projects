...
namespace Engine
{
    public static partial class RaymathF
    {
		...
		{
			Debug.Assert(iMin != iMax && iMin != iMax,"min and max can't be same");
        ...
        public static List<Vector2> Smooth(List<Vector2> input,float lerp = 0.25f)
        {
			var invert = 1 - lerp;
        ...
        public static Vector2 Vector2Rotate(Vector2 origin,Vector2 point,float angle)
        {
			var rad = angle;
		...
		public static Rectangle GetBorder(Vector2[] points)
		{
			...
			return new Rectangle(min.X, min.Y, max.X - min.X, max.Y - min.Y);//  RayUtils.CreateRectanglePoint(min, max);
		}
