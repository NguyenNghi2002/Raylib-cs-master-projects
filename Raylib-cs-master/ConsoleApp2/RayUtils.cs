...
namespace Raysharp
{
    ...
    public static  class RayUtils
    {
		...
		//---------------------------------------------------------------------------
		public static Rectangle SetTopLeft(this Rectangle rectangle,Vector2 point)
        {
            var moveError = point - new Vector2(rectangle.x, rectangle.y);
            rectangle.x = point.X;
            rectangle.y = point.Y;
            rectangle.width += moveError.X;
            rectangle.height += moveError.Y;
            return rectangle;
        }
