...
namespace Engine.UI
{
	public class Group : Element, ICullable
	{
		...
		{
			Insist.IsTrue(index >= 0,"invalid index");
		...
		{
			_previousBatcherTransform = Rlgl.rlGetMatrixTransform();
			Rlgl.rlPopMatrix();
			Rlgl.rlPushMatrix();
			Rlgl.rlMultMatrixf(transform);
		}
		...
		protected void ResetTransform()
		{
			Rlgl.rlPopMatrix();
			Rlgl.rlPushMatrix();
			Rlgl.rlMultMatrixf(_previousBatcherTransform);
		}
